 ...
 * @author Josh Hug, Xu Yang
 */
public class Router {

    private static Map<Long, Node> idToNodeMap;
    ...
    public static List<NavigationDirection> routeDirections(AugmentedStreetMapGraph g, List<Long> route) {
        ...
        // the route must have at least 2 nodes in order to be presented
        if(route.size() <= 1)
            return ans;

        // Build a map from id to a node if it doesn't exist
        if(idToNodeMap == null) {
            idToNodeMap = g.getMap();
        }

        // use the first item to initialize
        long preId = route.get(0);
        Node preNode = idToNodeMap.get(preId);
        double preBearing = 0;

        NavigationDirection cur = new NavigationDirection();
        // special case: get the street name of the first node
        cur.way = getStreetName(g, route.get(0), route.get(1));
        cur.direction = NavigationDirection.START;


        for(Long i : route.subList(1, route.size())) {

            Node curNode = idToNodeMap.get(i);
            double currentBearing = NavigationDirection.bearing(preNode.lon(), curNode.lon(), preNode.lat(), curNode.lat());
            String curStreetName = getStreetName(g, preId, i);

            if(!curStreetName.equals(cur.way)) {
                ans.add(cur);
                cur = new NavigationDirection();

                // update way
                cur.way = curStreetName;

                // update direction
                cur.direction = NavigationDirection.getDirection(preBearing, currentBearing);
            }

            // update distance
            cur.distance += g.estimatedDistanceToGoal(preId, i);
        ...
        // check the last item
        if(ans.size() == 0 || cur != ans.get(ans.size()-1))
            ans.add(cur);

        return ans;
    }

    // get the street name of the current name
    private static String getStreetName(AugmentedStreetMapGraph g, Long pre, Long cur) {

        List<WeightedEdge<Long>> edges = g.neighbors(pre);

        for(WeightedEdge<Long> e : edges) {
            if(cur.equals(e.to())) {
